...
if (isset($recherche)) {
    if (($lieu != 0) && ($annee != 0)) {
        for ($mois = 1; $mois <= 12; $mois++) {
            $info = $db->prepare("SELECT COUNT(*) AS nbVols FROM vols INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND YEAR((dateEvenement)=$annee) AND(lieu =$lieu)");
            $info->execute();
...
for ($mois = 1; $mois <= 12; $mois++) {
    $info = $db->prepare("SELECT COUNT(*) AS total FROM vols WHERE MONTH(dateEvenement) = $mois");
    $info->execute();
    $info->setFetchMode(PDO::FETCH_ASSOC);
    $tab = $info->fetch();
        ...
        <div class="secondaire">
            ...
            <div class="trois">
                ...
                <div class="fond_blanc">
                    <div class="text-form">
                        <h2>Statistiques des cas de vol</h2>
                        <form   method="POST">
