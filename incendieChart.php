...
if (isset($recherche)) {
    if (($lieu != 0) && ($annee != 0)) {
        for ($mois = 1; $mois <= 12; $mois++) {
            $info = $db->prepare("SELECT COUNT(*) AS nbIncendie FROM incendies INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND(lieu =$lieu) OR YEAR((dateEvenement)=$annee)");
            $info->execute();
            $info->setFetchMode(PDO::FETCH_ASSOC);
            $tab = $info->fetch();
            $total[] = $tab['nbIncendie'];
        }
    }
}
...
for ($mois = 1; $mois <= 12; $mois++) {
    $info = $db->prepare("SELECT COUNT(*) AS total FROM incendies WHERE MONTH(dateEvenement) = $mois");
    $info->execute();
    $info->setFetchMode(PDO::FETCH_ASSOC);
    $tab = $info->fetch();
        ...
        <div class="secondaire">
            ...
            <div class="trois">
                ...
                <div class="fond_blanc">
                    <div class="text-form">
                        <h2>Statistiques des cas d'Incendie</h2>
